...
// with HIDAPI_DISABLED defined to 1. You might want to do this for
...
hid_device_info :: struct {
	...
	/** Device Product ID */
	product_id:          c.uint16_t,
	/** Serial Number */
	serial_number:       [^]c.wchar_t,
	/** Device Release Number in binary-coded decimal,
        also known as Device Version Number */
	release_number:      c.uint16_t,
	/** Manufacturer String */
...
foreign lib {
	...
	// necessary, as it will be called automatically by hid_enumerate() and
	// any of the hid_open_*() functions if it is needed. This function should
	...
	// that anything has changed, but you can call hid_enumerate() to get an
	// updated device list.
	//
	...
	hid_device_change_count :: proc() -> c.uint32_t ---
	...
	// or crashing on bad drivers, but HINT_HIDAPI_ENUMERATE_ONLY_CONTROLLERS
	...
	hid_enumerate :: proc(vendor_id: c.uint16_t, product_id: c.uint16_t) -> ^hid_device_info ---
	...
	//                      NULL).
	// \returns a pointer to a hid_device object on success or NULL on
	//          failure; call GetError() for more information.
	//
	// \since This function is available since SDL 3.0.0.

	hid_open :: proc(vendor_id: c.uint16_t, product_id: c.uint16_t, serial_number: [^]c.wchar_t) -> ^hid_device ---
	...
	// hid_write() will always contain one more byte than the report contains.
	...
	//          GetError() for more information. If no packet was available to
	...
	// hid_send_feature_report() will always contain one more byte than the
	// report contains. For example, if a hid report is 16 bytes long, 17 bytes
	// must be passed to hid_send_feature_report(): the Report ID (or 0x0, for
	...
	//          still in the first byte), or -1 on on failure; call GetError()
	...
	hid_get_input_report :: proc(dev: ^hid_device, data: [^]u8, length: c.size_t) -> c.int ---
	...
	//
	// \param dev a device handle returned from hid_open().
	// \returns a pointer to the hid_device_info for this hid_device or NULL
	//          on failure; call GetError() for more information. This struct
	//          is valid until the device is closed with hid_close().
	//
	// \since This function is available since SDL 3.0.0.
	...
	hid_get_report_descriptor :: proc(dev: ^hid_device, buf: [^]u8, buf_size: c.size_t) -> c.int ---
	...
	hid_ble_scan :: proc(active: c.bool) ---
}

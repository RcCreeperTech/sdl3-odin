...
PathInfo :: struct {
	...
	size:        c.uint64_t, /**< the file size in bytes */
...
foreign lib {
	...
	CreateDirectory :: proc(path: cstring) -> c.bool ---
	...
	EnumerateDirectory :: proc(path: cstring, callback: EnumerateDirectoryCallback, userdata: rawptr) -> c.bool ---
	...
	RemovePath :: proc(path: cstring) -> c.bool ---
	...
	RenamePath :: proc(oldpath, newpath: cstring) -> c.bool ---
	...
	CopyFile :: proc(oldpath, newpath: cstring) -> c.bool ---
	...
	GetPathInfo :: proc(path: cstring, info: ^PathInfo) -> c.bool ---
	...
	//
	// \since This function is available since SDL 3.0.0.
	GlobDirectory :: proc(path, pattern: cstring, flags: GlobFlags, count: ^c.int) -> [^]cstring ---
}
